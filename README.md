...
**Critical:** WHERE filters are post-join operations. They see NULL values from LEFT JOINs and eliminate them.
...
|---------|---------|----------|
| LEFT JOIN + WHERE | Filter applied after join | Move filter to ON clause |
| col <> value | UNKNOWN treated as FALSE | Add `OR col IS NULL` |
| AVG/SUM/COUNT | Silently skips NULLs | Use COUNT(*) vs COUNT(col) intentionally |
| NOT IN (... NULL) | Three-valued logic → 0 rows | Use NOT EXISTS or explicit NULL check |

---

## Best Practices

1. **LEFT JOIN filters**: Right-table conditions go in ON clause
2. **Inequality comparisons**: Add `OR column IS NULL` explicitly
3. **Aggregations**: Always compare `COUNT(*)` vs `COUNT(column)`
4. **NOT IN**: Prefer NOT EXISTS when dealing with nullable columns

## Three-Valued Logic (3VL) Cheat Sheet
...
**WHERE clause rule:** Only rows where condition = TRUE are included. UNKNOWN is excluded.

---

## License
MIT - Free to use in your projects and training materials.
